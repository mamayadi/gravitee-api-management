...
class DefaultEndpointManagerTest {
    ...
    private static final String ENDPOINT_SHARED_CONFIG = "{ \"groupSharedConfig\": \"something in the shared config\"}";
    private static final String ENDPOINT_CONFIG = "{ \"config\": \"something\"}";
    private static final String ENDPOINT_GROUP_CONFIG = "{ \"groupConfig\": \"something for the group\"}";
    ...
    @Test
    void shouldStartAllEndpointsWithOverriddenGroupConfiguration() throws Exception {
        ...
        when(connectorFactory.createConnector(deploymentContext, ENDPOINT_CONFIG, ENDPOINT_GROUP_CONFIG))
        ...
        verify(connectorFactory, times(4)).createConnector(deploymentContext, ENDPOINT_CONFIG, ENDPOINT_GROUP_CONFIG);
    ...
    void shouldStartAllEndpointsWithSharedGroupConfiguration() throws Exception {
        ...
        when(connectorFactory.createConnector(deploymentContext, ENDPOINT_CONFIG, ENDPOINT_SHARED_CONFIG))
            .thenReturn(connector1, connector2, connector3, connector4);
        final DefaultEndpointManager cut = new DefaultEndpointManager(api, pluginManager, deploymentContext);
        cut.start();

        verify(pluginManager, times(4)).getFactoryById(ENDPOINT_TYPE);
        verify(connectorFactory, times(4)).createConnector(deploymentContext, ENDPOINT_CONFIG, ENDPOINT_SHARED_CONFIG);
        verify(connector1).start();
        verify(connector2).start();
        verify(connector3).start();
        verify(connector4).start();
    ...
    void shouldStartEndpointUsingGroupSharedConfiguration() throws Exception {
        ...
        verify(connectorFactory, times(2)).createConnector(deploymentContext, ENDPOINT_CONFIG, ENDPOINT_GROUP_CONFIG);
        // 2 connectors have been created with endpoint group configuration, cause endpoint2 inherits group configuration
        verify(connectorFactory, times(2)).createConnector(deploymentContext, ENDPOINT_CONFIG, ENDPOINT_SHARED_CONFIG);
    }
    ...
    private EndpointGroup buildEndpointGroup(Supplier<Endpoint> endpointSupplier) {
        ...
        endpointGroup.setSharedConfiguration(ENDPOINT_SHARED_CONFIG);
    ...
    private Endpoint buildEndpointUseOverriddenGroupConfiguration() {
        ...
        endpoint.setGroupConfiguration(ENDPOINT_GROUP_CONFIG);
        return endpoint;
    }
