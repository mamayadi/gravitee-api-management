...
public class FlowChain implements Hookable<ChainHook> {
    ...
    private static final String CONTEXT_ATTRIBUTES_FLOW_EXECUTION_NO_MATCH = "flowExecution.noMatch";
    private static final String EXECUTION_FAILURE_KEY_FAILURE = "FLOW_EXECUTION_NO_MATCH_FAILURE";
    ...
    public FlowChain(final String id, final FlowResolver flowResolver, final PolicyChainFactory policyChainFactory) {
        this(id, flowResolver, policyChainFactory, false);
    }
    ...
    public Completable execute(ExecutionContext ctx, ExecutionPhase phase) {
        return resolveFlows(ctx)
            .switchIfEmpty(
                Flowable.defer(
                    () -> {
                        Boolean previousExecutionMatchNoMatch = ctx.getInternalAttribute(CONTEXT_ATTRIBUTES_FLOW_EXECUTION_NO_MATCH);
                        ctx.setInternalAttribute(CONTEXT_ATTRIBUTES_FLOW_EXECUTION_NO_MATCH, true);
                        if (
                            ExecutionPhase.REQUEST == phase &&
                            interruptIfNoMatch &&
                            previousExecutionMatchNoMatch != null &&
                            previousExecutionMatchNoMatch
                        ) {
                            return ctx
            ...
            .doOnNext(
                flow -> {
                    ...
                    if (phase == ExecutionPhase.REQUEST) {
                        ctx.setInternalAttribute(CONTEXT_ATTRIBUTES_FLOW_EXECUTION_NO_MATCH, false);
                    }
                }
            )
